;;; pr-review.el --- Batched pull request reviews from Emacs -*- lexical-binding: t; -*-
...
;;
;; Commands: `pr-review-comment', `pr-review-list', `pr-review-submit',
;; `pr-review-refresh'.  Bind them yourself, or bind `pr-review-prefix-map'.
;;
...
(defcustom pr-review-gh-executable "gh"
...
(defun pr-review--gh ()
  ...
  (or (if (file-name-absolute-p pr-review-gh-executable)
          (and (file-executable-p pr-review-gh-executable) pr-review-gh-executable)
        (executable-find pr-review-gh-executable))
      (user-error "The `gh' CLI is not available (see `pr-review-gh-executable')")))

(defvar-local pr-review--comment-ctx nil
  "Context alist for the inline PR comment being composed.")
(defvar-local pr-review--submit-ctx nil
  "Context alist (root, review, event, number) for a review being submitted.")

;;;; Backend: GitHub GraphQL over the `gh' CLI

(defun pr-review--graphql (query variables)
...
or a GraphQL `errors' array (gh returns those with a non-zero exit)."
  (let ((gh (pr-review--gh))
        ...
        (out (generate-new-buffer " *pr-review-graphql*")))
    (unwind-protect
...
(defun pr-review--pr-ctx (&optional root)
...
Errors if the current branch has no associated open PR."
  (let* ((default-directory (or root (magit-toplevel)
         ...
         (out (generate-new-buffer " *pr-review-pr-view*")))
    (unwind-protect
        (if (/= 0 (call-process (pr-review--gh) nil out nil "pr" "view"
...
(defun pr-review--pending-review (pr-id)
...
It covers LINE, or the range START-LINE..LINE when START-LINE differs."
  (let* ((range (and start-line (/= start-line line)))
    ...
    (pr-review--graphql q vars)))

;;;; Commands

;;;###autoload
(defun pr-review-comment (beg end)
...
`pr-review-submit' or Forge's approve / request-changes.  With an active region,
...
hunk); the forge rejects anything outside it."
  ...
  (let* ((root (or (magit-toplevel) (user-error "Not inside a git repository")))
         ...
         (buf (get-buffer-create "*pr-review-comment*")))
    (with-current-buffer buf
      (erase-buffer)
      (when (fboundp 'gfm-mode) (gfm-mode))
      (setq pr-review--comment-ctx
...
(defun pr-review--comment-add ()
  ...
  (let* ((ctx pr-review--comment-ctx)
    ...
    (let* ((pr (pr-review--pr-ctx default-directory))
      ...
      (when (and (stringp (alist-get 'head pr))
        ...
        (message "Warning: worktree HEAD differs from PR head - run `pr-review-refresh'"))
      (pr-review--add-thread
       (pr-review--ensure-review (alist-get 'id pr) (alist-get 'head pr))
...
;;;###autoload
(defun pr-review-list ()
  "List the draft comments in this PR's pending review, read live from the forge."
  (interactive)
  (let* ((root (or (magit-toplevel) (user-error "Not inside a git repository")))
         (default-directory root)
         (pr (pr-review--pr-ctx root))
         (rid (pr-review--pending-review (alist-get 'id pr))))
    ...
    (let* ((q "query($r:ID!){ node(id:$r){ ... on PullRequestReview {
           ...
           (items (alist-get 'nodes (alist-get 'comments
                    (alist-get 'node (pr-review--graphql q `((r . ,rid)))))))
           (buf (get-buffer-create "*pr-review*")))
      (with-current-buffer buf
...
;;;###autoload
(defun pr-review-submit (&optional event)
...
verdict."
  ...
  (let* ((event (or event 'comment))
    ...
    (let ((buf (get-buffer-create "*pr-review-submit*")))
      (with-current-buffer buf
        (erase-buffer)
        (when (fboundp 'gfm-mode) (gfm-mode))
...
uncommitted changes (a review worktree should have none)."
  ...
  (let* ((root (or (magit-toplevel) (user-error "Not inside a git repository")))
    ...
    (let* ((pr (pr-review--pr-ctx root))
      ...
      (pr-review-diff-hl-set-base root (alist-get 'base pr))
      (let ((rid (pr-review--pending-review (alist-get 'id pr))))
        (if (not rid)
          ...
          (let* ((q "query($r:ID!){ node(id:$r){ ... on PullRequestReview {
            ...
            (message "Resynced PR #%s to %s - %d/%d draft comment(s) now outdated%s"
                     (alist-get 'number pr) (substring new-head 0 7)
                     n-out (length items)
                     (if (> n-out 0) " (`pr-review-list' to review)" ""))))))))

;;;; diff-hl: annotate the worktree with the PR's own changes

;;;###autoload
(defun pr-review-diff-hl-set-base (root base-ref)
...
;;;###autoload
(defun pr-review-diff-hl-set-from-pullreq (worktree-path pullreq)
  "Point diff-hl at PULLREQ's merge-base for the WORKTREE-PATH project.
Thin wrapper over `pr-review-diff-hl-set-base' that reads the base branch from
...
changed while LSP and jump-to-definition keep working on the real files."
  ...
  (let ((slot 'base-ref))
    (pr-review-diff-hl-set-base worktree-path (eieio-oref pullreq slot))))

;;;; Forge integration

(defun pr-review--maybe-submit-via-forge (event orig-fun args)
  "Submit the pending review with EVENT, else call ORIG-FUN with ARGS."
  (let* ((root (ignore-errors (magit-toplevel)))
         (rid (and root
                   (ignore-errors
                     (let ((default-directory root))
                       (pr-review--pending-review (alist-get 'id (pr-review--pr-ctx root))))))))
    (if rid
        (pr-review-submit event)
      (apply orig-fun args))))

(defun pr-review--forge-approve-advice (orig-fun &rest args)
  "Route `forge-approve-pullreq' (ORIG-FUN, ARGS) through the batch submitter."
  (pr-review--maybe-submit-via-forge 'approve orig-fun args))

(defun pr-review--forge-request-changes-advice (orig-fun &rest args)
  "Route `forge-request-changes' (ORIG-FUN, ARGS) through the batch submitter."
  (pr-review--maybe-submit-via-forge 'request-changes orig-fun args))

;;;###autoload
(define-minor-mode pr-review-forge-mode
...
behaviour when there is no pending review."
  :global t
  :group 'pr-review
  (if pr-review-forge-mode
      (progn
        (advice-add 'forge-approve-pullreq :around #'pr-review--forge-approve-advice)
        (advice-add 'forge-request-changes :around #'pr-review--forge-request-changes-advice))
    (advice-remove 'forge-approve-pullreq #'pr-review--forge-approve-advice)
    (advice-remove 'forge-request-changes #'pr-review--forge-request-changes-advice)))

;;;; Keymap

;;;###autoload
(defvar pr-review-prefix-map
  (let ((map (make-sparse-keymap)))
    (define-key map "c" #'pr-review-comment)
    (define-key map "l" #'pr-review-list)
    (define-key map "s" #'pr-review-submit)
    (define-key map "f" #'pr-review-refresh)
    map)
